...
<html lang="en">
  ...
  <body style="background: #ececec;">
    ...
    <div class="col-12 d-flex justify-content-center align-items-center mb-4" style="margin-top: 100px;">
      <div class="col-11"><h1><span>{{ $ogrenci->adsoyad }}</span> Bakiye Yükleme Geçmişi</h1></div>
    </div>
          ...
          <div style="overflow: auto;" class="col-12">
            ...
            <tbody>
              @foreach($yuklemeler as $yukleme)
                <tr>
                  <th scope="row">{{ $yukleme->id }}</th>
                  <td><a href="/bakiye_gecmisi/{{ $yukleme->ogrenci->id }}" class="text-secondary link-offset-2 link-underline link-underline-opacity-25">{{ $yukleme->ogrenci->adsoyad ?? ''}}</a></td>
                  <td>{{ $yukleme->gorevli->kullanici_adi ?? ''}}</td>
                  <td>{{ $yukleme->tutar }}</td>
                  <td>{{ $yukleme->created_at }}</td>
                </tr>
              @endforeach
            </tbody>
          </table>
